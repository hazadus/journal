...
from journal.models import Report, Task
    ...
    def handle(self, *args, **options):
        ...
        tasks = Task.objects.filter(
            ...
            | Q(
                ...
                is_completed=True,
                is_archived=False,
                completed__year=timezone.now().year,
                completed__month=timezone.now().month,
        ...
        for i, task in enumerate(tasks):
            ...
            worksheet = workbook.add_worksheet(
                name="{num}. {title}".format(
                    num=i + 1, title=sheet_title[:26]  # max len = 31
                )
            )
            ...
            task_stripped = md.markdown(
                task.body, extensions=["markdown.extensions.fenced_code"]
            )
            ...
            worksheet.write_datetime(
                1, 2, timezone.make_naive(task.created), date_format
            )
            ...
            for comment in task.comments.all():
                if not comment.is_archived:
                    comment_stripped = md.markdown(
                        comment.body, extensions=["markdown.extensions.fenced_code"]
                    )
                    ...
                    worksheet.write_datetime(
                        row, 2, timezone.make_naive(comment.created), date_format
                    )
                    row += 1
            ...
            if task.is_completed and task.completed:
                ...
                worksheet.write_datetime(
                    row, 2, timezone.make_naive(task.completed), date_format_header
                )
        ...
        report.attachment.name = "/{subdir}/{filename}".format(
            subdir=REPORTS_SUBDIR, filename=filename
        )
        report.save()
        ...
        Notification.send(
            ...
            verb_code=Notification.VERB_CODES.report_add,
            target=report,
        )
