...
from journal.templatetags.journal_filters import new_comments_count_for_user, is_user_acquainted, is_users_favorite
...
class TaskModelTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.task = Task.objects.create(
            author=cls.new_user, title=cls.task_title, category=new_category, body=cls.task_body,
        )
        # Add one `active` and one `archived` comment to the task:
        active_comment = Comment.objects.create(
            author=cls.new_user, task=cls.task, body=cls.comment_body_active, is_archived=False,
        )
        archived_comment = Comment.objects.create(
            author=cls.new_user, task=cls.task, body=cls.comment_body_archived, is_archived=True,
        )
    ...
    def test_is_user_acquainted(self):
        ...
        """
        self.assertEqual(
            is_user_acquainted(value=self.task, arg=self.new_user), False
        )
        ...
        self.task.comments.filter(is_archived=False).first().users_acquainted.add(self.new_user)
        self.assertEqual(
            is_user_acquainted(value=self.task, arg=self.new_user), True
        )
    ...
    def test_is_users_favorite(self):
        ...
        """
        self.assertEqual(
            is_users_favorite(value=self.task, arg=self.new_user), False
        )
        self.task.users_favorited.add(self.new_user)
        self.assertEqual(
            is_users_favorite(value=self.task, arg=self.new_user), True
        )
