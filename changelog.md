...
- 23:50 - Небольшие правки в UI админки и списка документов.

### 03.04.2023, Пн

- 22:30 - Весь код проекта переформатирован при помощи `black` и `isort`.
- 21:45 - В `core` добавлена модель `File` для хранения произвольных файлов на сайте. Файлы добавляются через админку.

### 19.03.2023, Вс

- 15:00 - В разеделе "Документы" добавлено отображение по категориям.

### 17.03.2023, Пт

- 11:30 - Обновление задач в Vue app при добавлении пользователем задачи в избранное (только для конкретного
          пользователя) - например, когда добавляется в избранное на другой вкладке браузера.
          Уменьшен шрифт в табах, margin-bottom под кнопками при редактировании задачи.
          При выборе другой задачи в списке, набранный текст комментария под задачей сбрасывается.
          В сайдбаре добавлены бейджи с суммарным количеством "новых" задач для видов "Таблица" и "Колонки".
          Бейджи в сайдбаре обновляются по событию "добавление в избранное".
          Vue app при выборе другой задачи из списка при включенном редактировании задачи, корректно отображается
          вновь выбранная задача, вместо редактора предыдущей задачи.

### 14.03.2023, Вт

- 18:00 - Добавлен endpoint `comment_edit_api` для редактирования комментариев и пара тестов к нему.
